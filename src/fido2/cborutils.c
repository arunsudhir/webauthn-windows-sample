    ...
    fido_attcred_t *attcred)
{
	...
	attcred->id.len = (size_t)id_len;  //DEBUG: TODO: Lambert: for TPM this coversion will not be needed (size_t)be16toh(id_len);
    ...
    fido_attcred_t *attcred, int *authdata_ext)
{
	...
	//authdata->sigcount = //DEBUG: TODO: Lambert: for TPM this coversion will not be needed be32toh(authdata->sigcount);
    ...
    fido_authdata_t *authdata, int *authdata_ext, fido_blob_t *hmac_secret_enc)
{
	...
	//authdata->sigcount = authdata->sigcount; //be32toh(authdata->sigcount);
