...
static int
fido_dev_make_cred_tx(fido_cred_t *cred)
{
	fido_blob_t	 f;
...
static int
fido_dev_make_cred_rx(fido_cred_t *cred, int ms)
{
...
static int
fido_dev_make_cred_wait(fido_cred_t *cred, int ms)
{
	int  r;

	if ((r = fido_dev_make_cred_tx(cred)) != FIDO_OK ||
	    (r = fido_dev_make_cred_rx(cred, ms)) != FIDO_OK)
		return (r);
...
int
fido_dev_make_cred(fido_cred_t *cred)
{
	...
	return (fido_dev_make_cred_wait(cred, -1));
}
...
fido_cred_set_rp(fido_cred_t *cred, const char *id, const char *name)
{
	...
	if (id != NULL && (rp->id = _strdup(id)) == NULL)
		goto fail;
	if (name != NULL && (rp->name = _strdup(name)) == NULL)
		goto fail;
    ...
    const char *icon)
{
	...
	if (name != NULL && (up->name = _strdup(name)) == NULL)
		goto fail;
	if (display_name != NULL &&
	    (up->display_name = _strdup(display_name)) == NULL)
		goto fail;
	if (icon != NULL && (up->icon = _strdup(icon)) == NULL)
		goto fail;
